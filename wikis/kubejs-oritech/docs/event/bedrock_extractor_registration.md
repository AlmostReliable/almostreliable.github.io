# Bedrock Extractor Registration Event

This event is a utility event to allow registering new materials and recipes mined by the [Bedrock Extractor](../recipe/machine/bedrock_extractor.md). Because the Bedrock Extractor mechanic depends on a recipe and a tag, this event handles both of these registrations at the same time.
...
- access in a server script via: `OritechEvents.bedrockExtractorRegistration`
- functions
    - `add(...)`
        ...
        - parameters:
            ...
            - `outputItem`
                - type: `Item`
                - required: yes
                - description: the item that should be the result of mining the resource node
            - `time`
                - type: `Integer`
                - required: no
                - default: `60`
...
To access the event, the first thing you need to do is to open an event listener for the `bedrockExtractorRegistration` event in a server script.

```js
OritechEvents.bedrockExtractorRegistration(event => {
    // ...
})
```
...
OritechEvents.bedrockExtractorRegistration(event => {
    ...
    event.add("glass", "cobblestone", "mymodpack:bedrock_extractor/cobblestone")

    // registers the dirt block as a resource node
    // adds a recipe that converts a dirt block into an iron ingot
    // requires 100 ticks / 5 seconds
    event.add("dirt", "iron_ingot", 100, "mymodpack:bedrock_extractor/iron_ingot")
})
```
