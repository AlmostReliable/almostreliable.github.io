...
- access in recipes event via: `event.recipes.oritech.fragment_forge`
- properties:
    - item inputs
        - role: input
        - required: yes
...
ServerEvents.recipes(event => {
    // removes all fragment forge recipes
    event.remove({ type: "oritech:fragment_forge" })

    // adds a recipe that converts a gold ingot into an iron ingot
    // requires 60 ticks by default
    event.recipes.oritech.fragment_forge().itemInputs("gold_ingot").itemOutputs("iron_ingot")
    ...
    event.recipes.oritech.fragment_forge().itemInputs("glass").itemOutputs("2x sand").time(40)
})
```
